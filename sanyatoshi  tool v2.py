...
if os.path.exists('.banner_840'):
    ...
    try:
        ...
        print(f'\n{LI_G}download....{RESET}')
        open('.banner_840','w')
        input(f'{LI_G}enter to enter: {RESET}')
        clear()
...
if os.path.exists('dataFile.txt'):
    try:
        ...
        sys.exit(f'\n{CYAN}{BOLD}[!] {RED}stop....{RESET}')
else: 
    clear()
print(f'{YELLOW}{BOLD}[?] {LI_G}download sanyatoshi tool v2.... {RESET}')
sleep(2)
clear()
print(f'{YELLOW}{BOLD}[#] {LI_G}success download sanyatoshi tool v2. {RESET}')
def getNumber():
    try:
        try:
            versioUR = requests.get('')
        ...
        banner()
        getNumber=input(f'{YELLOW}{BOLD}[~] {LI_G}Введите номер (пример 79999999999) : {RESET}')
        getNumber=getNumber.replace('+', '').replace('-', '').replace('(', '').replace(')', '').replace(' ', '')
        if getNumber.isdigit():return getNumber
        else:exit(f'{YELLOW}{BOLD}[!] {RED}"{RESET}{getNumber}{RED}" - неккоректный ввод номера!\n{RESET}')
    except KeyboardInterrupt:
        sys.exit(f'\n{YELLOW}{BOLD}[!] {RED}stop.....{RESET}')
...
try:
    ...
    print(f'{YELLOW}{BOLD}[~] {LI_G}собираю информацию.... {RESET}\n')
    try:
    ...
    try:
        ...
        for reg in region:
            ...
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные о Крае не известны{RESET}')  
            ...
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные об Области не известны{RESET}')  
            
            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Округ:{F_CL} {region["okrug"]}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные об Округе не известны{RESET}')  
        
            break

    except KeyboardInterrupt:sys.exit(f'\n{YELLOW}{BOLD}[!] {RED}stop....{RESET}')
    except:print(f'{YELLOW}{BOLD}[!] {RED}Данные Область/Край не известны{RESET}'); pass

    try:
        capital = num_P['capital']
        for c in capital:
            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Столица:{F_CL} {capital["name"]}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные об Сталице не известны{RESET}')  

            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Код домашнего номера столицы:{F_CL} +{str(capital["telcod"])}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные о Домашнем номере не известны{RESET}')  
            break

    except KeyboardInterrupt:sys.exit(f'\n{YELLOW}{BOLD}[!] {RED}stop..{RESET}')
    except:print(f'{YELLOW}{BOLD}[!] {RED}Данные Код/Столица не известны{RESET}'); pass

    try:
        data = num_P['0']
        for c in data:
            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Город:{F_CL} {data["name"]}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные о Городе не известны{RESET}')  

            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Район:{F_CL} {str(data["rajon"])}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные о Районе не известны{RESET}')  
            
            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Оператор:{F_CL} {data["oper_brand"]}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные об Операторе не известны{RESET}')  
        
            try:print(f'{YELLOW}{BOLD}[+] {LI_G}Номера:{F_CL} {str(data["def"])}{RESET}')
            except:print(f'{YELLOW}{BOLD}[!] {RED}Данные о Номерах оператора не известны{RESET}')  

            break
    except KeyboardInterrupt:sys.exit(f'\n{YELLOW}{BOLD}[!] {RED}stop....{RESET}')
    except:print(f'{YELLOW}{BOLD}[!] {RED}Данные Город/Оператор не известны{RESET}'); pass
            ...
            try:
                def get_url_name_avito(number):
                    ...
                    if h1.text == '503 Service Temporarily Unavailable':
                        print(f'{YELLOW}{BOLD}[!] {RED}Ваш запрос временно заблокирован подождите 6-15 минут.{RESET}')
                    else:
                        count = 0
                        h1T = h1.text.replace("  ","")
    ...
    sys.exit(f'\n{YELLOW}{BOLD}[!] {RED}Возможно, плохое интернет-соединение, попробуйте перезагрузить{RESET}\n{YELLOW}{BOLD}[+] {LI_G}Telegram:{YELLOW} @FELIX4{RESET}')